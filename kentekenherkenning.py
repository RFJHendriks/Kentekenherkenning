...
#     '''
    ...
    text = pytesseract.image_to_string(out, config='-c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789- --psm 13')
    textBlur = pytesseract.image_to_string(out, config='-c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 --psm 11')
    textOtsu = pytesseract.image_to_string(edgescolor, config='-c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 --psm 11')
    text = "".join(text.split()).replace(":","").replace("-", "")
    textBlur = "".join(textBlur.split()).replace(":","").replace("-", "")
    print('werkelijke license plate: ' + license_plate + ' gelezen kleur: ' + text + ' blurred: ' +textBlur + ' otsu: ' + textOtsu)
